...
extern "C" void Read_x1(u8 *, u64);
extern "C" void Read_x2(u8 *, u64);
extern "C" void Read_x3(u8 *, u64);
extern "C" void Read_x4(u8 *, u64);


int main() {
    {
        RepetitionTester tester{"Read_x1", total_size, 10, false};
        while(tester.start()) {
            auto mem = alloc_mem();
            Read_x1(mem, total_size);
            tester.stop();
        }
    }
    {
        RepetitionTester tester{"Read_x2", total_size, 10, false};
        while(tester.start()) {
            auto mem = alloc_mem();
            Read_x2(mem, total_size);
            tester.stop();
        }
    }
    {
        RepetitionTester tester{"Read_x3", total_size, 10, false};
        while(tester.start()) {
            auto mem = alloc_mem();
            Read_x3(mem, total_size);
            tester.stop();
        }
    }
    {
        RepetitionTester tester{"Read_x4", total_size, 10, false};
        while(tester.start()) {
            auto mem = alloc_mem();
            Read_x4(mem, total_size);
            tester.stop();
        }
    }
    return EXIT_SUCCESS;
